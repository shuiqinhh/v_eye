...
void capture_loop(const std::string & config_path, const std::string & output_folder)
{
  ...
  while (true) {
    ...
    auto img_2show = img.clone();

    std::vector<cv::Point2f> centers_2d;
    auto success = cv::findCirclesGrid(img, cv::Size(10, 7), centers_2d);  // 默认是对称圆点图案
    cv::drawChessboardCorners(img_2show, cv::Size(10, 7), centers_2d, success);  // 显示识别结果
    cv::resize(img_2show, img_2show, {}, 0.5, 0.5);  // 显示时缩小图片尺寸

    // 按“s”保存图片，按“q”退出程序
    cv::imshow("Press s to save, q to quit", img_2show);
